...
export async function discover(query: { skill?: string; tag?: string; q?: string }): Promise<Discovered[]> {
...
export async function findOne(
  ...
  const results = (await discover(query)).filter((r) => !exclude.includes(r.agent.name));
  const hit = results[0];
  if (!hit) return null;
